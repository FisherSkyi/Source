...
* simple functions (body consists of a single 
                    return statement)
* complex functions (body consists of a single 
                    return statement)
* loops and arrays
* JavaScript details:
  * JavaScripts for (let...) loops
  * return value of while and for loops
  * Proper tail calls
  * value/non-value-producing statements

Examples at the end of the program
*/

function is_value_producing(C) {
    return ! is_declaration(C) &&
           (! is_sequence(C) ||
            ! is_null(filter(C1 => is_value_producing(C1),
                             sequence_statements(C)))) &&
           (! is_block(C) || 
            is_value_producing(block_body(C)));
}
...
//        display_environment(E);
        ...
        if (is_literal(command)) {
            ...
            let body = sequence_statements(command);
            let value_produced = false;
            let reversed_commands = null;
            while (!is_null(body)) {
                const next = head(body);
                body = tail(body);
                if (is_value_producing(next)) {
                    if (value_produced) {
                        reversed_commands =
                ...
                reversed_commands = 
                pair(next, reversed_commands);
            }
            C = append(reverse(reversed_commands), C);
            ...
            C = pair(conditional_predicate(command),
                  pair(make_branch_instruction(
                    ...
                    C)); 
        } else if (is_conditional_statement(command)) {
            const consequent = conditional_consequent(command);
            const alternative = conditional_alternative(command);
            C = pair(conditional_predicate(command),
                  pair(make_branch_instruction(
                           display(is_value_producing(consequent),
                               "HERE:")
                           ? consequent
                           : make_sequence(
                               list(consequent,
                                    make_name("undefined"))),
                           is_value_producing(alternative)
                           ? alternative
                           : make_sequence(
                               list(alternative,
                                    make_name("undefined")))
            ...
            const pred = while_loop_predicate(command);
            C = pair(
                  make_name("undefined"),
                  pair(pred,
                       pair(make_while_instruction(
                               pred,
                               while_loop_body(command)),
                            C)));
        } else if (is_for_loop(command)) {
            const pred = for_loop_predicate(command);
            const init = for_loop_init(command);
            const update = for_loop_update(command);
            const body = for_loop_body(command);

            if (is_variable_declaration(init)) {
                const loop_var_symbol = declaration_symbol(init);
                C = pair(
                    make_block(
                      make_sequence(list(
                        init,
                        make_for_loop(
                          make_assignment(
                            loop_var_symbol,
                            make_name(loop_var_symbol)),
                          pred,
                          update,
                          make_block(
                              make_sequence(list(make_constant_declaration(
                                    make_name("_copy_of_" + loop_var_symbol),
                                    make_name(loop_var_symbol)
                                    ),
                                   make_block(
                                       make_sequence(list(make_constant_declaration(
                ...
                C = append(list(make_name("undefined"),
                                init,
                                make_pop_instruction(),
                                pred,
                                make_for_instruction(
                                    pred,
                                    update,
                                    body)
                               ),
        ...
        } else if (is_while_instruction(command)) {
            const body = while_instruction_body(command);
            const b = head(S);
            S = tail(S);
            if (is_truthy(b)) {
                if (is_value_producing(body)) {
                    C = append(list(make_pop_instruction(),
                                    while_instruction_body(command),
                                    while_instruction_predicate(command),
                                    command
                                   ),
                               C);
                } else {
        ...
        } else if (is_for_instruction(command)) {
            const body = for_instruction_body(command);
            const b = head(S);
            S = tail(S);
            if (is_truthy(b)) {
                if (is_value_producing(body)) {
                    C = append(list(make_pop_instruction(),
                                    for_instruction_body(command),
                                    for_instruction_update(command),
                                    make_pop_instruction(),
                                    for_instruction_predicate(command),
                                    command
                                   ),
                               C);
                } else {
    ...
    return is_null(S) ? undefined : head(S); 
}
...
function make_for_loop(init, pred, update, body) {
    return list("for_loop", init, pred, update, body);
}
...
function is_while_instruction(component) {	    
    return is_tagged_list(
             component, 
             "while_instruction");
}
function make_while_instruction(pred, body) {
    return list("while_instruction", pred, body);
}
function while_instruction_predicate(component) {
    return head(tail(component));
}
function while_instruction_body(component) {
    return head(tail(tail(component)));
}

// for instructions

function is_for_instruction(component) {	    
    return is_tagged_list(
             component, 
             "for_instruction");
}
function make_for_instruction(pred, update, body) {
    return list("for_instruction", pred, update, body);
}
function for_instruction_predicate(component) {
    return list_ref(component, 1);
}
function for_instruction_update(component) {
    return list_ref(component, 2);
}
function for_instruction_body(component) {
    return list_ref(component, 3);
}
